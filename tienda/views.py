...
def traer_fecha_valida(fecha, withtimezone = False):
    '''
        Retornará una fecha válida.
        Si queremos que tenga zonahoraria, withtimezone = True
    '''
    if fecha is None:
        return None
    
    partes = fecha.split("-") #2022      12     24
    if len(partes) != 3:
        return None
...
def v_index(request):
    ...
    fecha_e_inicio = traer_fecha_valida(fecha_e_inicio, withtimezone = True)

    fecha_e_fin = request.GET.get("fecha_e_fin", None)
    fecha_e_fin = traer_fecha_valida(fecha_e_fin, withtimezone = True)
